...
static Hole *getBlock( size_t size ) {
  ...
  if( h == NULL)
    return NULL;
  //Check the first node. If the size is sufficient copy the node.
  //Then set h = h.next and return the copied node.
  if(h->size >= size){
    Hole *ret = h;
    freeList = h->next;
    return ret;
  ...
  for ( h = freeList; h->next != NULL; h = h->next ){
      //If the next available node is big enough,
      //copy it, then set the current.next = next.next to unlink it.
      //Then return the copied node.
      //Otherwise keep going through the freeList.
      if(h->next->size >= size){
          Hole *ret = h->next;
          h->next = h->next->next;
          return ret;
  ...
  //If no sufficient block was found, return null.
  return NULL;
}
...
static void returnBlock( Hole *h ) {
  ...
  if ( pred ) {
    // merge h into its predecessor if there's no gap
    // between them.

    //Check if there is a suitable gap between pred and h
    if ( pred->mem + pred->size == (char *) h ) {
      //If so, increase pred.size accordingly.
      pred->size += h->size + sizeof( size_t );

      // And set the next field to the next.next field to skip h.
      pred->next = h->next;
    }
  }
